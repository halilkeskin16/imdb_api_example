...
import 'package:imdb_api_example/model/cast_member.dart';
...
class ApiServices {
  ...
  Future<List<CastMember>> getMovieCast(int movieId) async {
    ...
    final response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      final List<dynamic> jsonData = jsonDecode(response.body)['cast'];
      return jsonData.map((cast) => CastMember.fromJson(cast)).toList();
    } else {
